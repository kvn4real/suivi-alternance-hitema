...
(async () => {
  ...
  document
    .getElementById("entreprise-form")
    .addEventListener("submit", (e) => e.preventDefault());

  document
    .getElementById("btn-save-entreprise")
    .addEventListener("click", saveEntreprise);
})();
...
function applyView() {
  document
    ...
    .classList.toggle("hidden", CURRENT_VIEW !== "table");

  document
    .getElementById("view-kanban")
    .classList.toggle("hidden", CURRENT_VIEW !== "kanban");
...
function renderToolbar() {
  ...
  const densityBtn = `
    <button
      class="btn btn-ghost density-toggle"
      id="btn-toggle-density"
      title="Basculer la densité du tableau"
    >
      <span class="icon">${ICONS.chart}</span>
      <span id="density-label">Vue compacte</span>
    </button>
  `;
  ...
  if (CURRENT_PROFILE.role === "candidate") {
    ...
    document
      .getElementById("btn-add-entreprise")
      .addEventListener(
        "click",
        () => openEntrepriseForm(null)
      );

    document
      .getElementById("btn-open-import")
      .addEventListener(
        "click",
        openImportModal
      );
    ...
    toolbar.innerHTML = `
      <p
        ...
        "
      >
        Consultation des entreprises suivies.
      </p>
  ...
  const isCompact =
    localStorage.getItem("entreprises-density") ===
    "compact";
  ...
  document
    .getElementById("btn-toggle-density")
...
async function loadCampagnesOptions() {
  const selectForm =
    document.getElementById("ent-campagne_id");
...
function renderKanban() {
  ...
  const rows =
    kanbanFilteredEntreprises();

  const isCandidate =
    CURRENT_PROFILE.role === "candidate";
    ...
    .forEach(col => {
      ...
      col.addEventListener(
        ...
        async e => {
          ...
          const ent =
            ALL_ENTREPRISES.find(
              x => x.id === draggedId
            );

          if (
            !ent ||
            ent.statut === newStatut
          ) {
            return;
          }
...
function exportCsv() {
  ...
  showToast(
    `${rows.length} entreprise(s) exportée(s) en CSV.`,
    "success"
  );
}
...
function exportPdf() {
  ...
  if (
    ...
    showToast(
      "Le module PDF n'a pas pu se charger.",
      "error"
    );
  ...
  doc.text(
    "Suivi des candidatures — Alternance Suite",
    40,
    36
  );
  ...
  showToast(
    `${rows.length} entreprise(s) exportée(s) en PDF.`,
    "success"
  );
}
...
async function openDetail(id) {
  ...
  document.getElementById(
    ...
    <div class="detail-row">
      <span class="k">Secteur</span>
      <span class="v">
        ${escapeHtml(e.secteur || "—")}
      </span>
    </div>
  ...
  // on ne bloque plus l'ouverture de la fiche
  // si la fonction des lettres n'existe pas.
  if (
    typeof loadLettresForEntreprise ===
    "function"
  ) {
    await loadLettresForEntreprise(
      e.id
    );
  } else {
    ...
    if (lettersList) {
      lettersList.innerHTML = `
        <p
          ...
          "
        >
          Aucun historique de lettre disponible.
        </p>
      `;
    }
  }
...
async function loadLettresForEntreprise(
  entrepriseId
) {
  ...
  try {
    ...
    /*
     * On cherche les lettres liées à cette entreprise.
     *
     * Cette partie utilise la table "lettres".
     * Si ta table contient entreprise_id,
    ...
    if (error) {
      ...
      list.innerHTML = `
        <p
          ...
          "
        >
          Impossible de charger l'historique des lettres.
        </p>
      `;
    ...
    if (!data || !data.length) {
      ...
      list.innerHTML = `
        <p
          ...
          "
        >
          Aucune lettre générée pour cette entreprise.
        </p>
      `;
      ...
      data.map(letter => {
        ...
        return `
          <div
            ...
            onclick="window.location.href='lettres.html?id=${encodeURIComponent(
              letter.id
            )}'"
          >
    ...
    console.error(
      "Erreur inattendue chargement lettres :",
      error
    );
...
async function loadNotes(
  ...
  const { data, error } =
    await supabaseClient
      .from("notes")
      .select(
        "*, author:profiles!notes_author_id_fkey(prenom,nom,role)"
      )
      .eq(
        "entreprise_id",
        entrepriseId
  ...
  try {
    ...
    if (
      ...
      window.location.href =
        `lettres.html?id=${encodeURIComponent(
          data.lettre.id
        )}`;
      ...
      showToast(
        "La lettre a été générée, mais son identifiant est introuvable.",
        "error"
      );
    }
  ...
  reader.onload =
    ev => {
      ...
      if (!rows.length) {
        ...
        errEl.textContent =
          "Aucune entreprise valide détectée (colonne 'nom' requise).";
...
async function confirmImport() {
  ...
  await logActivity(
    ...
    null,
    "Import CSV",
    `${payload.length} entreprises importées`
  );

  showToast(
    `${payload.length} entreprise(s) importée(s).`,
    "success"
  );
